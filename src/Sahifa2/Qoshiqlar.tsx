...
function Qoshiqlar() {
  ...
  async function Dta() {
    try{
        ...
        const d=await fetch(`https://api.spotify.com/v1/playlists/${params.id}?additional_types=10`,
        {
            headers:{
...
useEffect(()=>{
  ...
  return (
    <Contner>
        ...
        <MsC>
          <Ms><span style={{marginRight:'15px'}}>#</span> TITLE</Ms>
          <Ms>ALBUM</Ms>
          <Ms>DATE ADDED</Ms>
    ...
    
        {
          malumot?.tracks.items.map((a:any, index:any):any=>{
            return(
              <Muzc onClick={()=>dispach(Omborchi.actions.url(a.track.preview_url))}>
                  <Index>{index+1}</Index>
